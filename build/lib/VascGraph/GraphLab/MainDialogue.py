...
class MainDialogue(HasTraits):
    ...
    Add_rest_source=Button(label='Rest as sources')
    Add_rest_sink=Button(label='Rest as sinks')
    ...
    #others ##########################
    TypeValue={'Artery': 1, 'Vein':2, 'Capillary':3}
...
# -------------------------------------------------------
    ...
    def _Add_source_fired(self):
        if int(self.NodeID) not in self.Sources and int(self.NodeID) not in self.Sinks: 
            ...
            self.Graph.node[int(self.NodeID)]['source']=1
            self.Graph.node[int(self.NodeID)]['inflow']=1
    ...
    def _Add_sink_fired(self): 
        if int(self.NodeID) not in self.Sinks and int(self.NodeID) not in self.Sources: 
            ...
            self.Graph.node[int(self.NodeID)]['sink']=1
            self.Graph.node[int(self.NodeID)]['outflow']=1  
        
            
    def _Add_rest_source_fired(self):
        ...
        if nx.is_directed(self.Graph):
            ...
            jnodes=self.Graph.GetJuntionNodes([1])
        ...
        for i in jnodes:
            ...
            self.Graph.node[i]['source']=1
            self.Graph.node[i]['inflow']=1    


            
    def _Add_rest_sink_fired(self):
    ...
    def _Remove_source_fired(self): 
        if int(self.NodeID) in self.Sources: 
            ...
            del(self.Graph.node[int(self.NodeID)]['source'])
            del(self.Graph.node[int(self.NodeID)]['inflow'])  
            
    def _Remove_sink_fired(self): 
        if int(self.NodeID) in self.Sinks: 
            self.Sinks.remove(int(self.NodeID))
            del(self.Graph.node[int(self.NodeID)]['sink'])
            del(self.Graph.node[int(self.NodeID)]['outflow'])  
    ...
    labeling_group =  Group(
                        ...
                        Group(     
                                Group(Item('Add_source', show_label=False),
                                      ...
                                      Item('Add_rest_source', show_label=False),
                                      Item('Add_rest_sink', show_label=False),
