...
class Viscosity(polyfit3D):
    ...
    def __init__(self, order=3):
        '''
        build regression model for vescosity based on vessel type and diameter
        Boas et. al. 2008 (NeuroImage) "A vascular anatomical network Model ..."
        '''
        typee=[1, 1, 1, 1, 1, 1 ,3 ,2 ,2, 2, 2, 2, 2] # 1 artery, 2 vein, 3 capillary
...
class Hematocrit(polyfit3D):
    ...
    def __init__(self, order=4):
        ...
        '''
        build regression model for Hematocrit based on vessel type and diameter
        Boas et. al. 2008 (NeuroImage) "A vascular anatomical network Model ..."
        '''        
        typee=[1, 1, 1, 1, 1, 1 ,3 ,2 ,2, 2, 2, 2, 2] # 1 artery, 2 vein, 3 capillary
        diam=[30.5, 24.4, 19.5, 15.6, 12.5, 10, 8, 12, 15, 18.7, 23.4, 29.3, 36.6]
...
class InitFlowSimulation:
    ...
    def PrepareGraph(self, arteriol_pressure=None,
                             venule_pressure=None): 
        
        '''
        add flow and pressure labels on the sink and source nodes 
        '''
        
        # set inlet and outlet pressure values 
        if arteriol_pressure is not None:
            self.arteriol_pressure=arteriol_pressure
            
        if venule_pressure is not None:  
            self.venule_pressure=venule_pressure
                
        # ------ label graph sink/source nodes ------#
        print('Prepare graph ...')
        ...
        if self.metadata is not None:
            ...
            sources, sinks = self.Graph.GetSourcesSinks()
            if len(sources)<1:
                wrong=wrong+' sources '
            if  len(sinks)<1:
                wrong=wrong+' sinks '
        ...
        if wrong !='':
            print('Cannot find '+wrong+' in the graph!')
            return                     
        
        # obtain end nodes only without source/sink
        ...
        for i in sources_sinks: end_nodes.remove(i) # end nodes only
        

#        # label inflow and outflow edges ont he graph
